            ...
            if (paymentUserAdvertisement.isPresent()) {
                ...
                if (payment.getStatus().equals(PaymentStatusEnum.PAID) || payment.getStatus().equals(PaymentStatusEnum.CONCLUDED)) {
                    ...
                    if (paymentControllSeller.isPresent()) {
                        ...
                        Double amountApplyTax = (transaction.getAmount() * paymentControllSeller.get().getAmountTax()) / 100;
                        Double amountLiquid = (transaction.getAmount() - amountApplyTax);
                        user.setBalance(user.getBalance() + amountLiquid);
